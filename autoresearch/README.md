...
| Sector | Tickers | Sharpe | Return | OOS Sharpe | Status |
|--------|---------|--------|--------|------------|--------|
| **Tech** | AAPL, NVDA, MSFT, GOOGL, TSLA | **2.02** | +58.5% | 1.41 | Tuned |
| **Equipment** | AMAT, ASML, LRCX, KLAC, TEL | **1.86** | +95.0% | **2.35** | Tuned |
| **Memory** | MU, WDC, STX | **2.44** | +229.6% | — | Baseline (tech params) |
| Power | VST, CEG, NRG | 0.04 | +4.0% | — | Needs tuning |
| Energy | XOM, CVX, OXY, SLB, EOG | 0.03 | +3.8% | — | Does not generalize |

**Key wins:** Per-sector params framework (`params_equipment.py`, `params_power.py`, `params_memory.py`), equipment tuned from 1.63→1.86 (+14%), memory crushes with tech params (2.44), overnight autoresearch loop ready.
...
|------|---------|----------------|
| `params.py` | Tech tunable knobs | AI agent (tech loop) |
| `params_equipment.py` | Equipment sector knobs | AI agent (equipment loop) |
| `params_power.py` | Power sector knobs | AI agent (power loop) |
| `params_memory.py` | Memory sector knobs | AI agent (memory loop) |
| `evaluate.py` | Runs one experiment. `--params`, `--start`/`--end`, `--tickers`, `--prices-path` | Nobody (fixed) |
| `fast_backtest.py` | Deterministic backtester (no LLM) | Nobody (fixed) |
| `cache_signals.py` | Data caching. `--prices-path` for sector caches | Nobody (run once per sector) |
| `oos_check.py` | Out-of-sample validation | Nobody (run to check robustness) |
| `cross_asset_check.py` | Cross-asset test (tech vs energy) | Nobody (run to check generalization) |
| `program.md` | AI agent instructions (tech) | Nobody (fixed) |
| `program_equipment.md` | AI agent instructions (equipment) | Nobody (fixed) |
| `run_overnight_equipment.sh` | Overnight loop entry point | Nobody (run to start) |
| `results.tsv` | Tech experiment log | Appended automatically |
| `results_equipment.tsv` | Equipment experiment log | Appended automatically |
| `cache/` | Cached prices + signals. `prices.json`, `prices_equipment.json`, etc. | Generated by cache_signals.py |
...
Cross-asset testing confirms the strategy is **sector-specific**. We now have a **per-sector params framework** — each sector has its own `params_<sector>.py` and can be tuned independently.

### Current State

| Sector | Tickers | Sharpe | Return | Max DD | OOS | Params file | Status |
|--------|---------|--------|--------|--------|-----|-------------|--------|
| **Tech** | AAPL, NVDA, MSFT, GOOGL, TSLA | **2.02** | +58.5% | -8.2% | 1.41 | `params.py` | Tuned |
| **Equipment** | AMAT, ASML, LRCX, KLAC, TEL | **1.86** | +95.0% | -17.5% | **2.35** | `params_equipment.py` | Tuned |
| **Memory** | MU, WDC, STX | **2.44** | +229.6% | -18.7% | — | `params_memory.py` | Baseline (tech params) |
| **Power** | VST, CEG, NRG | 0.04 | +4.0% | -16.7% | — | `params_power.py` | Needs tuning |
| **Energy** | XOM, CVX, OXY, SLB, EOG | 0.03 | +3.8% | -17.8% | — | — | Does not generalize |

**Equipment tuning journey:** 1.63 (tech baseline) → 1.70 (ADX 40, EMA 5/21/55) → 1.77 (MOM_3M 0.1) → 1.79 (SIG_BULL 0.25) → 1.85 (SIG 0.28, SIG_BEAR -0.15, VOL_Z 1.5) → **1.86** (BOLLINGER 30, RISK_EXTREME 0.60). OOS improved from 2.09 → **2.35**.

**Memory:** Tech params generalize extremely well — no tuning needed yet. Highest return of any sector (+229%).

**Power:** Mean-reversion bias (RSI 30/70, lower momentum) improved from -0.02 → 0.04. Full autoresearch pending.

### Planned Sectors

| Sector | Tickers | Behavioral regime |
|--------|---------|-------------------|
| **EDA** | SNPS, CDNS | Long-duration compounder, low vol |
| **Tokenization rails** | COIN, HOOD, CRCL | Crypto-correlated, high beta |
| **Hyperscalers** | META, MSFT, AMZN, GOOGL | Momentum, AI capex driven |
| **Foundry** | TSM | Geopolitical + capex cycle |
...
Equipment baseline to beat: **Sharpe 1.86, OOS 2.35**. Power and memory: `--params autoresearch.params_power` or `--params autoresearch.params_memory`.

---

## Summary: What We Built

- **4 sector param files** — tech, equipment, power, memory
- **3 price caches** — `prices.json`, `prices_equipment.json`, `prices_power.json`, `prices_memory.json`
- **Overnight loop** — `program_equipment.md` + `run_overnight_equipment.sh` for autonomous equipment tuning
- **~50+ equipment experiments** — MOM_3M, signal thresholds, vol regime, Bollinger, risk extreme
- **2 sectors tuned** — tech (2.02), equipment (1.86)
- **1 sector crushing** — memory (2.44 with tech params, no tuning)
