...
class TestRateLimiting:
    ...
    @patch("src.data.providers.financial_datasets.time.sleep")
    @patch("src.data.providers.financial_datasets.requests.post")
    def test_handles_post_rate_limiting(self, mock_post, mock_sleep):
        ...
        mock_post.assert_has_calls([call(url, headers=headers, json=json_data), call(url, headers=headers, json=json_data)])
    ...
    @patch("src.data.providers.financial_datasets._cache")
    @patch("src.data.providers.financial_datasets.time.sleep")
    @patch("src.data.providers.financial_datasets.requests.get")
    def test_full_integration(self, mock_get, mock_sleep, mock_cache):
        ...
        with patch.dict(os.environ, {"FINANCIAL_DATA_PROVIDER": "financialdatasets", "FINANCIAL_DATASETS_API_KEY": "test-key"}):
            # Call get_prices
