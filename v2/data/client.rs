use super::models::{
    CompanyFacts, CompanyNews, Earnings, EarningsRecord, FinancialMetrics, InsiderTrade, Price,
};
...
impl FDClient {
    ...
    pub async fn get_earnings_history(&self, ticker: &str, limit: u32) -> Vec<EarningsRecord> {
        let url = format!(
            "https://api.financialdatasets.ai/earnings/?ticker={}&limit={}",
            ticker, limit
        );
        let res_json = match ai_hedge_fund::tools::api::make_api_request(
            &url,
            "GET",
            None,
...
impl DataClient for FDClient {
    ...
    async fn get_financial_metrics(
        ...
        ai_hedge_fund::tools::api::get_financial_metrics(
            ticker,
            end_date,
            period,
            limit,
    ...
    async fn get_news(
        ...
        ai_hedge_fund::tools::api::get_company_news(
            ticker,
            end_date,
            start_date,
            limit,
    ...
    async fn get_insider_trades(
        ...
        ai_hedge_fund::tools::api::get_insider_trades(
            ticker,
            end_date,
            start_date,
            limit,
    ...
    async fn get_company_facts(&self, ticker: &str) -> Option<CompanyFacts> {
        ...
        let res_json =
            ai_hedge_fund::tools::api::make_api_request(&url, "GET", None, self.api_key.as_deref())
                .await
                .ok()?;
    ...
    async fn get_earnings(&self, ticker: &str) -> Option<Earnings> {
        let url = format!(
            "https://api.financialdatasets.ai/earnings/?ticker={}&limit=1",
            ticker
        );
        let res_json =
            ai_hedge_fund::tools::api::make_api_request(&url, "GET", None, self.api_key.as_deref())
