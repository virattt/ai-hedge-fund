...
| A run finishes but shows no decisions (or the backtester errors) | The flow has no **Portfolio Manager** node, or its nodes aren't connected. Wire your analysts into a Portfolio Manager (**Stock Input → analyst(s) → Portfolio Manager**) or start from a built-in template — see [Using the AI hedge fund](#using-the-ai-hedge-fund). |
