    ...
    <div className="flex-grow overflow-auto text-primary scrollbar-thin scrollbar-thumb-ramp-grey-700">
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-8">
          <div className="text-muted-foreground text-sm">{t('flows.loading')}</div>
        </div>
      ) : (
        <Accordion
          type="multiple"
        ...
        <div className="text-center py-8 text-muted-foreground text-sm">
          {flows.length === 0 ? (
            <div className="space-y-2">
              ...
              <div>{t('flows.noneSaved')}</div>
              <div className="text-xs">{t('flows.createFirst')}</div>
            </div>
          ) : (
            t('flows.noneMatch')
          )}
