...
start_services() {
    ...
    cd ..
    poetry run uvicorn app.backend.main:app --reload --host 127.0.0.1 --port 8765 > "$LOG_DIR/backend.log" 2>&1 &
    BACKEND_PID=$!
    cd app
    ...
    print_status "Backend (API): http://localhost:8765"
    print_status "API Documentation: http://localhost:8765/docs"
