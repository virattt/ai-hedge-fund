...
impl BacktestService {
    pub async fn run_backtest_streaming(
        ...
        let mut portfolio = Portfolio::new(tickers.clone(), initial_capital, req.base.margin_requirement);
        ...
        while current_dt <= end_dt {
            ...
            let agent_output = match controller.run_agent(
                tickers.clone(),
                &lookback_start,
                &current_date_str,
                &portfolio,
                ...
                Ok(out) => out,
                Err(e) => {
                    tx.send(Err(anyhow::anyhow!("Error running agents: {}", e))).await.ok();
                    return Ok(());
                }
            };
            
            let decisions_json = agent_output.decisions.clone().unwrap_or_else(|| serde_json::json!({}));
            let decisions: HashMap<String, ai_hedge_fund::agents::portfolio_manager::PortfolioDecision> = serde_json::from_value(decisions_json.clone()).unwrap_or_default();
            ...
            let total_value = ai_hedge_fund::backtesting::valuation::calculate_portfolio_value(&portfolio, &daily_prices);
            let exposures = ai_hedge_fund::backtesting::valuation::compute_exposures(&portfolio, &daily_prices);
            ...
            let mut computed_metrics = PerformanceMetrics::default();
            if portfolio_values.len() > 3 {
                if let Some(comp) = calculator.compute_metrics(&portfolio_values) {
                    computed_metrics = comp;
                }
            }
            
            // Build BacktestDayResult response
            let decisions_val: HashMap<String, serde_json::Value> = serde_json::from_value(decisions_json.clone()).unwrap_or_default();
