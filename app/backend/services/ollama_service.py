...
class OllamaService:
    ...
    async def download_model(self, model_name: str) -> Dict[str, any]:
        ...
        try:
            ...
            message = (
                f"Model {model_name} downloaded successfully" if success else f"Failed to download model {model_name}"
            )
            return {"success": success, "message": message}
    ...
    def _stop_unix_process(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["pgrep", "-f", "ollama serve"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            )
    ...
    async def _stream_model_download(self, model_name: str) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            error_data = {"status": "error", "message": f"Error downloading model {model_name}", "error": str(e)}
    ...
    def _process_download_progress(self, progress, model_name: str) -> Optional[Dict[str, any]]:
        ...
        if (
            ...
            progress_data.update(
                {"percentage": percentage, "bytes_downloaded": progress.completed, "total_bytes": progress.total}
            )
