...
class FlowRunRepository:
    ...
    def create_flow_run(self, flow_id: int, request_data: Dict[str, Any] = None) -> HedgeFundFlowRun:
        ...
        flow_run = HedgeFundFlowRun(
            flow_id=flow_id, request_data=request_data, run_number=run_number, status=FlowRunStatus.IDLE.value
        )
    ...
    def get_active_flow_run(self, flow_id: int) -> Optional[HedgeFundFlowRun]:
        ...
        return (
            self.db.query(HedgeFundFlowRun)
            .filter(HedgeFundFlowRun.flow_id == flow_id, HedgeFundFlowRun.status == FlowRunStatus.IN_PROGRESS.value)
            .first()
        )
    ...
    def _get_next_run_number(self, flow_id: int) -> int:
        ...
        max_run_number = (
            self.db.query(func.max(HedgeFundFlowRun.run_number)).filter(HedgeFundFlowRun.flow_id == flow_id).scalar()
        )
