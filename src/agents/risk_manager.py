...
def risk_management_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        current_position_value = portfolio.get("positions", {}).get(ticker, 0) * current_price

        # Calculate total portfolio value using stored prices
        total_portfolio_value = portfolio.get("cash", 0) + sum((portfolio.get("cost_basis", {}).get(t, 0) * portfolio.get("positions", {}).get(t, 0)) for t in portfolio.get("cost_basis", {}))
