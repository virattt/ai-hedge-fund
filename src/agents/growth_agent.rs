...
pub async fn growth_analyst_agent(state: &mut AgentState, agent_id: &str) -> Result<()> {
    ...
    for ticker in &tickers {
        // Fetch raw data
        let metrics = get_financial_metrics(ticker, end_date, "ttm", 12, api_key).await.unwrap_or_default();
        if metrics.len() < 4 {
            println!("Not enough financial metrics for {}", ticker);
            continue;
        }

        let insider_trades = get_insider_trades(ticker, end_date, None, 1000, api_key).await.unwrap_or_default();
        let most_recent_metrics = &metrics[0];

        // 1. Growth trends
        let growth_trends = analyze_growth_trends(&metrics);

        // 2. Growth valuation
        let valuation_metrics = analyze_valuation(most_recent_metrics);

        // 3. Margin trends
        let margin_trends = analyze_margin_trends(&metrics);
        ...
        // Aggregate scores
        let scores = vec![
            (growth_trends.score, 0.40),
            (valuation_metrics.score, 0.25),
            (margin_trends.score, 0.15),
            (insider_conviction.score, 0.10),
            (financial_health.score, 0.10),
        ];

        let weighted_score: f64 = scores.iter().map(|&(s, w)| s * w).sum();
        ...
        let confidence = ((weighted_score - 0.5).abs() * 2.0 * 100.0).round() as u32;
...
fn calculate_trend(data: &[Option<f64>]) -> f64 {
...
pub fn analyze_growth_trends(metrics: &[FinancialMetrics]) -> GrowthTrendsResult {
    ...
    let rev_trend = calculate_trend(&rev_growth);
    let eps_trend = calculate_trend(&eps_growth);
    let fcf_trend = calculate_trend(&fcf_growth);
...
pub fn analyze_margin_trends(metrics: &[FinancialMetrics]) -> MarginTrendsResult {
    ...
    let gm_trend = calculate_trend(&gross_margins);
    let om_trend = calculate_trend(&operating_margins);
    let nm_trend = calculate_trend(&net_margins);
