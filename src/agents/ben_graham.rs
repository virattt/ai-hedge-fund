...
pub async fn ben_graham_agent(state: &mut AgentState, agent_id: &str) -> Result<()> {
    ...
    if let Some(obj) = analyst_signals.as_object_mut() {
        obj.insert(
            agent_id.to_string(),
            serde_json::Value::Object(graham_analysis),
        );
    }
...
pub fn analyze_earnings_stability(
    ...
    if financial_line_items.is_empty() {
        return GrahamSubResult {
            score: 0,
            details: "Insufficient data for earnings stability analysis".to_string(),
        };
    }

    let eps_vals: Vec<f64> = financial_line_items
        .iter()
...
pub fn analyze_financial_strength(financial_line_items: &[LineItem]) -> GrahamSubResult {
    ...
    if financial_line_items.is_empty() {
        return GrahamSubResult {
            score: 0,
            details: "No data for financial strength analysis".to_string(),
        };
    }
    ...
    if current_liabilities > 0.0 {
        ...
        if current_ratio >= 2.0 {
            score += 2;
            details.push(format!(
                "Current ratio = {:.2} (>=2.0: solid)",
                current_ratio
            ));
        } else if current_ratio >= 1.5 {
            score += 1;
            details.push(format!(
                "Current ratio = {:.2} (moderately strong)",
                current_ratio
            ));
        } else {
            details.push(format!(
        ...
        details.push(
            "Cannot compute current ratio (missing or zero current_liabilities).".to_string(),
        );
    }
    ...
    if total_assets > 0.0 {
        ...
        if debt_ratio < 0.5 {
            score += 2;
            details.push(format!(
                "Debt ratio = {:.2}, under 0.50 (conservative)",
                debt_ratio
            ));
        } else if debt_ratio < 0.8 {
            score += 1;
            details.push(format!(
                "Debt ratio = {:.2}, somewhat high but could be acceptable",
                debt_ratio
            ));
        } else {
            details.push(format!(
                "Debt ratio = {:.2}, quite high by Graham standards",
                debt_ratio
            ));
        }
    } else {
    ...
    let div_periods: Vec<f64> = financial_line_items
        .iter()
    ...
    if !div_periods.is_empty() {
        ...
        if div_paid_years > 0 {
            if div_paid_years >= (div_periods.len() / 2 + 1) {
                score += 1;
                details.push(
                    "Company paid dividends in the majority of the reported years.".to_string(),
                );
            } else {
...
pub fn analyze_valuation_graham(
    financial_line_items: &[LineItem],
    market_cap: f64,
) -> GrahamSubResult {
    ...
    if net_current_asset_value > 0.0 && shares_outstanding > 0.0 {
        ...
        if net_current_asset_value > market_cap {
            ...
            score += 2;
            details.push(
                "NCAV Per Share >= 2/3 of Price Per Share (moderate net-net discount).".to_string(),
            );
        }
    } else {
        details.push(
            "NCAV not exceeding market cap or insufficient data for net-net approach.".to_string(),
        );
    }
    ...
    if eps > 0.0 && book_value_ps > 0.0 {
        ...
        details
            .push("Unable to compute Graham Number (EPS or Book Value missing/<=0).".to_string());
    }
    ...
    if let Some(gn) = graham_number {
        if shares_outstanding > 0.0 {
            ...
            if current_price > 0.0 {
                ...
                details.push(format!(
                    "Margin of Safety (Graham Number) = {:.2}%",
                ...
                if margin_of_safety > 0.5 {
                    ...
                    details.push(
                        "Price close to or above Graham Number, low margin of safety.".to_string(),
                    );
                }
            }
        }
