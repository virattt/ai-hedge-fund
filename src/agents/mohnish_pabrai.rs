...
pub async fn mohnish_pabrai_agent(state: &mut AgentState, agent_id: &str) -> Result<()> {
    ...
    for ticker in &tickers {
        // Fetch raw data
        let metrics = get_financial_metrics(ticker, end_date, "annual", 8, api_key).await.unwrap_or_default();
    ...
    if let Some(obj) = analyst_signals.as_object_mut() {
        obj.insert(agent_id.to_string(), serde_json::Value::Object(pabrai_analysis));
    }
...
pub fn analyze_downside_protection(financial_line_items: &[LineItem]) -> PabraiSubResult {
    ...
    if financial_line_items.is_empty() {
        return PabraiSubResult { score: 0, details: "Insufficient data".to_string() };
    }
    ...
    if current_liabilities > 0.0 {
        ...
        if current_ratio >= 2.0 {
            score += 2;
            details.push(format!("Strong liquidity (current ratio {:.2})", current_ratio));
        } else if current_ratio >= 1.2 {
            score += 1;
            details.push(format!("Adequate liquidity (current ratio {:.2})", current_ratio));
        } else {
            details.push(format!("Weak liquidity (current ratio {:.2})", current_ratio));
        }
    }
    ...
    let fcf_values: Vec<f64> = financial_line_items.iter().filter_map(|li| li.free_cash_flow).collect();
    if fcf_values.len() >= 3 {
    ...
    PabraiSubResult { score: score.min(10), details: details.join("; ") }
}
...
pub fn analyze_pabrai_valuation(financial_line_items: &[LineItem], market_cap: f64) -> PabraiValResult {
    if financial_line_items.is_empty() || market_cap <= 0.0 {
        return PabraiValResult { score: 0, details: "Insufficient data".to_string(), fcf_yield: None, normalized_fcf: None };
    }
    ...
    if fcf_values.len() < 3 {
        return PabraiValResult { score: 0, details: "Insufficient FCF history".to_string(), fcf_yield: None, normalized_fcf: None };
    }
    ...
    if normalized_fcf <= 0.0 {
        return PabraiValResult { score: 0, details: "Non-positive normalized FCF".to_string(), fcf_yield: None, normalized_fcf: Some(normalized_fcf) };
    }
    ...
    if fcf_yield > 0.10 {
        score += 4;
        details.push(format!("Exceptional value: {:.1}% FCF yield", fcf_yield * 100.0));
    } else if fcf_yield > 0.07 {
        score += 3;
        details.push(format!("Attractive value: {:.1}% FCF yield", fcf_yield * 100.0));
    } else if fcf_yield > 0.05 {
        score += 2;
        details.push(format!("Reasonable value: {:.1}% FCF yield", fcf_yield * 100.0));
    } else if fcf_yield > 0.03 {
        score += 1;
        details.push(format!("Borderline value: {:.1}% FCF yield", fcf_yield * 100.0));
    } else {
    ...
    if !capex_to_revenue.is_empty() {
        ...
        if avg_ratio < 0.05 {
            score += 2;
            details.push(format!("Asset-light: Avg capex {:.1}% of revenue", avg_ratio * 100.0));
        } else if avg_ratio < 0.10 {
            score += 1;
            details.push(format!("Moderate capex: Avg capex {:.1}% of revenue", avg_ratio * 100.0));
        } else {
            details.push(format!("Capex heavy: Avg capex {:.1}% of revenue", avg_ratio * 100.0));
        }
    }

    PabraiValResult { score: score.min(10), details: details.join("; "), fcf_yield: Some(fcf_yield), normalized_fcf: Some(normalized_fcf) }
}

pub fn analyze_double_potential(financial_line_items: &[LineItem], market_cap: f64) -> PabraiSubResult {
    let mut score = 0;
    let mut details = Vec::new();
    ...
    let revenues: Vec<f64> = financial_line_items.iter().filter_map(|li| li.revenue).collect();
    let fcfs: Vec<f64> = financial_line_items.iter().filter_map(|li| li.free_cash_flow).collect();
    ...
    if revenues.len() >= 3 {
        ...
        if older_rev > 0.0 {
            ...
            if rev_growth > 0.15 {
                score += 2;
                details.push(format!("Strong revenue trajectory ({:.1}%)", rev_growth * 100.0));
            } else if rev_growth > 0.05 {
                score += 1;
                details.push(format!("Modest revenue growth ({:.1}%)", rev_growth * 100.0));
            }
        }
    }
    ...
    if let Some(fcf_yield) = val_tmp.fcf_yield {
        if fcf_yield > 0.08 {
            score += 3;
            details.push("High FCF yield can drive doubling via retained cash/buybacks".to_string());
        } else if fcf_yield > 0.05 {
            score += 1;
