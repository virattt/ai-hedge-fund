...
def estimate_maintenance_capex(financial_line_items: list) -> float:
    ...
    latest_capex = (
        abs(financial_line_items[0].capital_expenditure) if financial_line_items[0].capital_expenditure else 0
    )
...
def analyze_book_value_growth(financial_line_items: list) -> dict[str, any]:
    ...
    book_values = [
        ...
        and hasattr(item, "outstanding_shares")
        and item.shareholders_equity
        and item.outstanding_shares
    ]
