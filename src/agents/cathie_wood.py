...
def analyze_disruptive_potential(metrics: list, financial_line_items: list) -> dict:
    ...
    if len(revenues) >= 3:  # Need at least 3 periods to check acceleration
        ...
        for i in range(len(revenues) - 1):
            if revenues[i] and revenues[i + 1]:
                growth_rate = (
                    (revenues[i + 1] - revenues[i]) / abs(revenues[i])
                    if revenues[i] != 0
                    else 0
                )
        ...
        if len(growth_rates) >= 2 and growth_rates[-1] > growth_rates[0]:
            score += 2
            details.append(
                f"Revenue growth is accelerating: {(growth_rates[-1] * 100):.1f}% vs {(growth_rates[0] * 100):.1f}%"
            )
    ...
    if len(revenues) >= 2 and len(operating_expenses) >= 2:
        ...
        opex_growth = (operating_expenses[-1] - operating_expenses[0]) / abs(
...
def analyze_innovation_growth(metrics: list, financial_line_items: list) -> dict:
    ...
    if rd_expenses and revenues and len(rd_expenses) >= 2:
        ...
        rd_growth = (
            (rd_expenses[-1] - rd_expenses[0]) / abs(rd_expenses[0])
            if rd_expenses[0] != 0
            else 0
        )
    ...
    if op_margin_vals and len(op_margin_vals) >= 2:
        ...
        if op_margin_vals[-1] > 0.15 and margin_trend > 0:
            score += 3
            details.append(
                f"Strong and improving operating margin: {(op_margin_vals[-1] * 100):.1f}%"
            )
        elif op_margin_vals[-1] > 0.10:
            score += 2
            details.append(
                f"Healthy operating margin: {(op_margin_vals[-1] * 100):.1f}%"
            )
        elif margin_trend > 0:
            score += 1
    ...
    if capex and revenues and len(capex) >= 2:
        ...
        capex_growth = (
            (abs(capex[-1]) - abs(capex[0])) / abs(capex[0]) if capex[0] != 0 else 0
        )
