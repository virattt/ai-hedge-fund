...
use crate::data::models::{LineItem, InsiderTrade, CompanyNews, Price};
...
pub async fn stanley_druckenmiller_agent(state: &mut AgentState, agent_id: &str) -> Result<()> {
    ...
    for ticker in &tickers {
        ...
        let _metrics = get_financial_metrics(ticker, &end_date_str, "annual", 5, api_key).await.unwrap_or_default();
        ...
        let insider_trades = get_insider_trades(ticker, &end_date_str, None, 50, api_key).await.unwrap_or_default();
        let company_news = get_company_news(ticker, &end_date_str, None, 50, api_key).await.unwrap_or_default();
        let prices = get_prices(ticker, &start_date_str, &end_date_str, api_key).await.unwrap_or_default();
    ...
    if let Some(obj) = analyst_signals.as_object_mut() {
        obj.insert(agent_id.to_string(), serde_json::Value::Object(druck_analysis));
    }
...
pub fn analyze_growth_and_momentum(financial_line_items: &[LineItem], prices: &[Price]) -> DruckSubResult {
    if financial_line_items.len() < 2 {
        return DruckSubResult { score: 0.0, details: "Insufficient financial data for growth analysis".to_string() };
    }
    ...
    let revenues: Vec<f64> = financial_line_items.iter().filter_map(|fi| fi.revenue).collect();
    if revenues.len() >= 2 {
        ...
        if older_rev > 0.0 && latest_rev > 0.0 {
            ...
            if rev_growth > 0.08 {
                raw_score += 3;
                details.push(format!("Strong annualized revenue growth: {:.1}%", rev_growth * 100.0));
                ...
                details.push(format!("Minimal/negative revenue growth: {:.1}%", rev_growth * 100.0));
            }
        } else {
            details.push("Older revenue is zero/negative; can't compute revenue growth.".to_string());
        }
    } else {
    ...
    // 2. EPS Growth (CAGR)
    let eps_values: Vec<f64> = financial_line_items.iter().filter_map(|fi| fi.earnings_per_share).collect();
    if eps_values.len() >= 2 {
        ...
        if older_eps > 0.0 && latest_eps > 0.0 {
            ...
            if eps_growth > 0.08 {
                raw_score += 3;
                details.push(format!("Strong annualized EPS growth: {:.1}%", eps_growth * 100.0));
                ...
                details.push(format!("Minimal/negative annualized EPS growth: {:.1}%", eps_growth * 100.0));
            }
        } else {
    ...
    if prices.len() > 30 {
        ...
        if close_prices.len() >= 2 {
            ...
            if start_price > 0.0 {
                ...
                if pct_change > 0.50 {
                    raw_score += 3;
                    details.push(format!("Very strong price momentum: {:.1}%", pct_change * 100.0));
                } else if pct_change > 0.20 {
                    raw_score += 2;
                    details.push(format!("Moderate price momentum: {:.1}%", pct_change * 100.0));
                } else if pct_change > 0.0 {
                    raw_score += 1;
                    details.push(format!("Slight positive momentum: {:.1}%", pct_change * 100.0));
                } else {
                    details.push(format!("Negative price momentum: {:.1}%", pct_change * 100.0));
                }
            }
        }
    } else {
    ...
    DruckSubResult { score: final_score, details: details.join("; ") }
}
...
pub fn analyze_insider_activity(insider_trades: &[InsiderTrade]) -> DruckSubResult {
    ...
    if insider_trades.is_empty() {
        ...
        return DruckSubResult { score, details: details.join("; ") };
    }
    ...
    for trade in insider_trades {
        if let Some(shares) = trade.transaction_shares {
            if shares > 0.0 { buys += 1; } else if shares < 0.0 { sells += 1; }
        }
    }
    ...
    DruckSubResult { score, details: details.join("; ") }
}

/// Sentiment: negative keyword ratio in news titles.
pub fn analyze_sentiment(news_items: &[CompanyNews]) -> DruckSubResult {
    if news_items.is_empty() {
        return DruckSubResult { score: 5.0, details: "No news data; defaulting to neutral sentiment".to_string() };
    }

    let negative_keywords = ["lawsuit", "fraud", "negative", "downturn", "decline", "investigation", "recall"];
...
pub fn analyze_risk_reward(financial_line_items: &[LineItem], prices: &[Price]) -> DruckSubResult {
    if financial_line_items.is_empty() || prices.is_empty() {
        return DruckSubResult { score: 0.0, details: "Insufficient data for risk-reward analysis".to_string() };
    }
    ...
    // 1. Debt-to-Equity
    let debt_values: Vec<f64> = financial_line_items.iter().filter_map(|fi| fi.total_debt).collect();
    let equity_values: Vec<f64> = financial_line_items.iter().filter_map(|fi| fi.shareholders_equity).collect();

    if !debt_values.is_empty() && !equity_values.is_empty() {
        let recent_debt = debt_values[0];
        let recent_equity = if equity_values[0].abs() > 1e-9 { equity_values[0] } else { 1e-9 };
    ...
    if prices.len() > 10 {
        ...
        if close_prices.len() > 10 {
            ...
            if !daily_returns.is_empty() {
                ...
                if stdev < 0.01 {
                    raw_score += 3;
                    details.push(format!("Low volatility: daily returns stdev {:.2}%", stdev * 100.0));
                } else if stdev < 0.02 {
                    raw_score += 2;
                    details.push(format!("Moderate volatility: daily returns stdev {:.2}%", stdev * 100.0));
                } else if stdev < 0.04 {
                    raw_score += 1;
                    details.push(format!("High volatility: daily returns stdev {:.2}%", stdev * 100.0));
                } else {
                    details.push(format!("Very high volatility: daily returns stdev {:.2}%", stdev * 100.0));
                }
            }
        }
    } else {
...
pub fn analyze_druckenmiller_valuation(financial_line_items: &[LineItem], market_cap: f64) -> DruckSubResult {
    if financial_line_items.is_empty() || market_cap <= 0.0 {
        return DruckSubResult { score: 0.0, details: "Insufficient data to perform valuation".to_string() };
    }
