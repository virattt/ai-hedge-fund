...
pub async fn peter_lynch_agent(state: &mut AgentState, agent_id: &str) -> Result<()> {
    ...
    if let Some(obj) = analyst_signals.as_object_mut() {
        obj.insert(
            agent_id.to_string(),
            serde_json::Value::Object(lynch_analysis),
        );
    }
    ...
    if revenues.len() >= 2 {
        ...
        if older > 0.0 {
            ...
            if rev_growth > 0.25 {
                ...
                details.push(format!(
                    "Flat or negative revenue growth: {:.1}%",
                    rev_growth * 100.0
                ));
            }
        }
    ...
    if eps_values.len() >= 2 {
        ...
        if older.abs() > 1e-9 {
            ...
            if eps_growth > 0.25 {
                ...
                details.push(format!(
                    "Minimal or negative EPS growth: {:.1}%",
                    eps_growth * 100.0
                ));
            }
        }
...
pub fn analyze_lynch_fundamentals(financial_line_items: &[LineItem]) -> LynchSubResult {
    if financial_line_items.is_empty() {
        return LynchSubResult {
            score: 0.0,
            details: "Insufficient fundamentals data".to_string(),
        };
    }
...
pub fn analyze_lynch_valuation(
    ...
    if eps_values.len() >= 2 {
        ...
        if beginning > 0.0 {
            ...
            details.push(format!(
                "Annualized EPS growth rate: {:.1}%",
                eps_growth_rate.unwrap() * 100.0
            ));
        }
    }
...
pub fn analyze_sentiment(news_items: &[CompanyNews]) -> LynchSubResult {
    if news_items.is_empty() {
        return LynchSubResult {
            score: 5.0,
            details: "No news data; default to neutral sentiment".to_string(),
        };
    }
