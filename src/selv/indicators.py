...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df = df.copy()
    df["rsi"] = ta.rsi(df["close"], length=14)          # RSI 14 periods 🡒 traders’ classic choice [oai_citation:7‡Stack Overflow](https://stackoverflow.com/questions/57006437/calculate-rsi-indicator-from-pandas-dataframe?utm_source=chatgpt.com)
    macd = ta.macd(df["close"], fast=12, slow=26, signal=9)  # MACD line, Signal, Hist. [oai_citation:8‡Medium](https://medium.com/%40financial_python/building-a-macd-indicator-in-python-190b2a4c1777?utm_source=chatgpt.com)
...
class StrategyState:
    ...
    def __init__(self, rsi_period=14, macd_fast=12, macd_slow=26, macd_signal=9):
        ...
        self.gain_queue, self.loss_queue = deque(maxlen=rsi_period), deque(maxlen=rsi_period)
    ...
    def _update_rsi(self, close: float) -> float | None:
        if len(self.gain_queue) == self.rsi_period:
            ...
            self.avg_gain = (self.avg_gain * (self.rsi_period - 1) + old_gain) / self.rsi_period
            self.avg_loss = (self.avg_loss * (self.rsi_period - 1) + old_loss) / self.rsi_period
        # add newest
    ...
    def update(self, ohlcv: dict) -> dict | None:
        ...
        if self.position == 0:                              # flat
            if macd_line > signal_line and rsi_val and rsi_val > 55:   # long entry [oai_citation:9‡algobulls.github.io](https://algobulls.github.io/pyalgotrading/strategies/rsi_macd_crossover/?utm_source=chatgpt.com)
                action = "LONG"
