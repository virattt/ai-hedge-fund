...
def run_strategy_on_df(df: pd.DataFrame) -> dict:
    ...
    position = 0; equity = 1.0; peak = 1.0; dds = []
    equity_ts = []
    entry_price = entry_time = None
    for ts, row in df.iterrows():
        price = row["close"]
        if position == 0:
            if long_entry.loc[ts]:   position, entry_price, entry_time = 1, price, ts
            elif short_entry.loc[ts]: position, entry_price, entry_time = -1, price, ts
        else:
    ...
    sharpe = returns.mean()/returns.std()*np.sqrt(365 * 24 * 60) if len(returns) > 1 else 0
