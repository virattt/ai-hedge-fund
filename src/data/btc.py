...
async def fetch_candles_parallel(exchange, symbol, timeframe, since, limit):
    ...
    async def fetch_window(start_ts):
        return await exchange.fetch_ohlcv(symbol, timeframe=timeframe, since=start_ts, limit=limit)
    results = []
...
logging.info(f"Starting to fetch historical data for {symbol} from {one_year_ago.strftime('%Y-%m-%d')} to present.")

# Use async fetch to populate all_candles
all_candles = asyncio.run(fetch_candles_parallel(exchange, symbol, timeframe, since, batch_limit))
