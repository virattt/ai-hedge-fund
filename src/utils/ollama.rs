...
pub fn is_ollama_installed() -> bool {
    ...
    match Command::new(cmd).arg("ollama").stdout(Stdio::null()).stderr(Stdio::null()).status() {
...
pub async fn is_ollama_server_running() -> bool {
    ...
    match client.get(format!("{}/api/tags", url)).timeout(Duration::from_secs(2)).send().await {
...
pub async fn start_ollama_server() -> bool {
    ...
    let spawned = if cfg!(target_os = "windows") {
        Command::new("cmd").args(&["/C", "ollama serve"]).stdout(Stdio::null()).stderr(Stdio::null()).spawn()
    } else {
        Command::new("ollama").arg("serve").stdout(Stdio::null()).stderr(Stdio::null()).spawn()
    };
...
pub async fn ensure_ollama_and_model(model_name: &str) -> bool {
    ...
    println!("\x1b[33mModel {} is not available locally.\x1b[0m", model_name);
