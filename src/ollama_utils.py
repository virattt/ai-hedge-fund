...
def main():
    ...
    download_parser.add_argument(
        "model", help="Model name to download (e.g., gemma3:4b)"
    )

    # Reinstall command
    reinstall_parser = subparsers.add_parser(
        "reinstall", help="Delete and re-download a model"
    )
    reinstall_parser.add_argument(
        "model", help="Model name to reinstall (e.g., gemma3:4b)"
    )
    ...
    if args.command == "list":
        ...
        if delete_model(args.model):
            print(
                f"{Fore.GREEN}Successfully deleted model {args.model}.{Style.RESET_ALL}"
            )
        else:
            sys.exit(1)
        ...
        if download_model(args.model):
            print(
                f"{Fore.GREEN}Successfully downloaded model {args.model}.{Style.RESET_ALL}"
            )
        else:
            sys.exit(1)
        ...
        if not delete_model(args.model):
            print(
                f"{Fore.RED}Failed to delete model {args.model}. Continuing with download anyway...{Style.RESET_ALL}"
            )

        # Then download it again
        if download_model(args.model):
            print(
                f"{Fore.GREEN}Successfully reinstalled model {args.model}.{Style.RESET_ALL}"
            )
        else:
            sys.exit(1)
